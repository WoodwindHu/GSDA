...
def main():
    ...
    # model
    # model_path = os.path.join('Pretrained', cfg.arch, 'defense_no_latent', str(cfg.npoint), 'model_best.pth.tar')
    ...
    for i, (adv_pc, gt_label, attack_label) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            aa = torch.max(defense_output,1)[1].data.cpu()
            if aa in ten_label_indexes2:
                bb = ten_label_indexes[ten_label_indexes2.index(aa)]
            else:
                bb = aa
        ...
        if gt_label.view(-1) == attack_label.view(-1):
            ...
            defense_success = (bb == gt_label.view(-1)).sum()
            attack_still_success = (bb == attack_label.view(-1)).sum()
        num_defense_success += defense_success
        # print(num_defense_success)
        ...
        if (i+1) % cfg.print_freq == 0:
            print('[{0}/{1}]  attack success: {2:.2f} still attack success: {3:.2f} avg drop num: {4:.2f}'.format(
                i+1, len(test_loader), (1-num_defense_success/float(cnt))*100, num_attack_still_success/float(cnt)*100,num_drop_point/float(cnt)))


    final_acc = num_defense_success/float(test_loader.dataset.__len__())*100
    final_attack_acc = num_attack_still_success/float(test_loader.dataset.__len__())*100
