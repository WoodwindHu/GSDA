...
import torch_dct
from Lib.utility import estimate_perpendicular, _compare, farthest_points_sample, pad_larger_tensor_with_index_batch, dct, idct
from Lib.loss_utils import norm_l2_loss, chamfer_loss, pseudo_chamfer_loss, hausdorff_loss, curvature_loss, uniform_loss, _get_kappa_ori, _get_kappa_adv, spectral_loss
...
def _forward_step(net, pc_ori, input_curr_iter, normal_ori, ori_kappa, target, scale_const, cfg, targeted, v):
...
def attack(net, input_data, cfg, i, loader_len, saved_dir=None):
    ...
    if (cfg.arch != "PointNet" and cfg.arch != "PointNetPP2"):
        ten_label_indexes = [17, 9, 36, 20, 3, 16, 34, 38, 23, 15]
        ten_label_indexes2 = [0, 2, 4, 5, 8, 22, 30, 33, 35, 37]
        for tmp_i, label in enumerate(gt_labels):
            #print(tmp_i,label)
            gt_labels[tmp_i] = ten_label_indexes2[ten_label_indexes.index(label[0])]
            #print(gt_labels[tmp_i])
    if pc.size(3) == 3:
    ...
    for search_step in range(cfg.binary_max_steps):
        ...
        for step in range(cfg.iter_max_steps):
            if cfg.is_partial_var:
                ...
                if step == 0:
                    ...
                    if cfg.spectral_attack:
                        ...
                        from Attacker import spectral_attack
                        K = cfg.KNN
                        if cfg.test_time:
                            import time
                        ...
                        if cfg.test_time:
                            print("gft time:", time.time() - time_begin)
                        u = u.unsqueeze(-1)
                        ...
                        if cfg.optim == 'adam':
                            ...
                            assert False, 'Not support such optimizer.'
                    elif cfg.dct3d_attack:
                        coords = pc_ori.clone()
                        features = pc_ori.clone()
                        dct_offset = torch.zeros(b, 3, 128, 128, 128).cuda()
                        nn.init.normal_(dct_offset, mean=0, std=1e-3)
                        dct_offset.requires_grad_()
                        voxel_features, voxel_coords = voxelization(features, coords)
                        x_ = torch_dct.dct_3d(voxel_features)
                        # x_.requires_grad = False
                        if cfg.optim == 'adam':
                            optimizer = optim.Adam([dct_offset], lr=cfg.lr)
                        elif cfg.optim == 'sgd':
                            optimizer = optim.SGD([dct_offset], lr=cfg.lr)
                        else:
                            assert False, 'Not support such optimizer.'
                    else:
                        if cfg.optim == 'adam':
            ...
            if cfg.spectral_attack:
                ...
                input_all = idct(x_ + offset)
            elif cfg.dct3d_attack:
                voxel_features = torch_dct.idct_3d(x_ + dct_offset)
                input_all = F.trilinear_devoxelize(voxel_features, voxel_coords, 128, True)
            else:
                ...
                for k in range(b):
                    ...
                    if attack_success[k] and (metric <best_loss[k]):
                        ...
                        if cfg.spectral_attack:
                            ...
                            #best_factor_[k] = factor_[k].clone()
                        elif cfg.dct_attack or cfg.dct3d_attack:
                            best_x_[k] = x_[k].clone().transpose(0,1)
                            best_factor_[k] = offset[k].clone().transpose(0,1)
                            best_gft[k] = best_x_[k] + best_factor_[k]
            ...
            _, normal_curr_iter, loss, loss_n, cls_loss, dis_loss, hd_loss, nor_loss, constrain_loss, info = _forward_step(net, pc_ori, input_curr_iter, normal_ori, kappa_ori, target, scale_const, cfg, targeted, v)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='GSDA Point Cloud Attacking')
    #------------Model-----------------------
